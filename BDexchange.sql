...
MySQL - 5.5.5-10.4.8-MariaDB : Database - exchange
*********************************************************************
*/
...
insert  into `busqueda`(`idbusqueda`,`fecha`,`palabra_clave`,`precio`,`usuario_idusuario`) values (1,'marzo','celular','',33),(2,'marzo','celular',NULL,33),(4,'marzo','celular',NULL,33);
...
insert  into `categoria`(`idcategoria`,`categoria`) values (1,'Tecnologia'),(2,'vehiculos'),(3,'Video juegos'),(4,'Deportes'),(5,'Hogar'),(6,'Joyeria'),(7,'Maquinaria'),(10,'Ropa'),(11,'Muebles');
...
CREATE TABLE `comentarios` (
  ...
  CONSTRAINT `comentarios_ibfk_1` FOREIGN KEY (`idproducto_id`) REFERENCES `producto` (`idproducto`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `interesados` (
  ...
  CONSTRAINT `interesados_ibfk_2` FOREIGN KEY (`idproducto_id`) REFERENCES `producto` (`idproducto`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=utf8;

/*Data for the table `interesados` */

insert  into `interesados`(`idinteresados`,`idproducto_id`,`idusuario_id`,`confirmado`) values (7,6,56,NULL),(9,18,33,NULL);
...
CREATE TABLE `producto` (
  ...
  `precio_estimado` varchar(100) DEFAULT NULL,
...
insert  into `producto`(`idproducto`,`nombre`,`cantidad`,`descripcion`,`precio_estimado`,`marca`,`tiempo_uso`,`usuario_idusuario`,`categoria_idcategoria`,`tiempo_publicacion_idtiempo_publicacion`,`imagen1`,`imagen2`,`imagen3`,`disponible`,`FechaPublicación`) values (6,'cedula','11','oijiojiojoij','0','jiojiojoi','',33,1,NULL,'img/productos/product01.png','img/productos/product02.png','img/productos/product03.png',0,'2019-11-25'),(7,'cedula','Seleccione una opcion','oijoijioj','0','','',56,10,NULL,'img/productos/t2.jpg','img/productos/t3.jpg','img/productos/t4.jpg',0,'2019-11-25'),(18,'chcuhi','1','ddjdiudjui','478487','deded','',33,10,NULL,'img/productos/product01.png','img/productos/product01.png','img/productos/product01.png',1,'2019-12-03');
...
CREATE TABLE `solicitud_intercambio` (
  ...
  CONSTRAINT `fk_solicitud_intercambio_producto1` FOREIGN KEY (`producto_idproducto`) REFERENCES `producto` (`idproducto`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `solicitud_intercambio_ibfk_1` FOREIGN KEY (`idinteresados_id`) REFERENCES `interesados` (`idinteresados`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8;

/*Data for the table `solicitud_intercambio` */

insert  into `solicitud_intercambio`(`idsolicitud_intercambio`,`oferta`,`estado`,`notificacion`,`producto_idproducto`,`idinteresados_id`) values (5,NULL,NULL,NULL,7,7);
...
insert  into `usuario`(`idusuario`,`usuario`,`telefono`,`correo`,`password`,`genero`,`ciudad_id_ciudad`) values (33,'Jonathan','3132958131','camanojhonatan@gmail.com','Jf12QqYilanVtU4VPfK7Ow==','Masculino',11001),(56,'d','cjduij','cecec','ho','Masculino',1),(57,'usuario','telefono','correo','clave','genero',1),(58,'ewdiuhduih','dede','dewdewd','eded','duhui',1),(59,'cijoiji','3189898090','samuelalejandro@gmail.com','G/d9LNerp8I88HwnmmDpOg==','Masculino',11001);

/* Procedure structure for procedure `agregarpublicacion` */

/*!50003 DROP PROCEDURE IF EXISTS  `agregarpublicacion` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `agregarpublicacion`(
in Pronombre varchar(100),
IN Procantidad VARCHAR(10),
IN Prodescripcion VARCHAR(150),
IN Proprecio VARCHAR(50),
IN Promarca VARCHAR(50),
IN Prousuario int,
IN Procategoria int,
IN Proimagen1 VARCHAR(150),
IN Proimagen2 VARCHAR(150),
IN Proimagen3 VARCHAR(150),
IN Prodisponible boolean,
IN Profecha date
)
begin
insert into producto (nombre,cantidad,descripcion,precio_estimado,marca,usuario_idusuario,categoria_idcategoria,imagen1,imagen2,imagen3,disponible,FechaPublicación)
VALUES(Pronombre,Procantidad,Prodescripcion,Proprecio,Promarca,Prousuario,Procategoria,Proimagen1,Proimagen2,Proimagen3,Prodisponible,Profecha);
...
in USUusuario varchar(50),
IN USUcorreo VARCHAR(50),
IN USUclave VARCHAR(50),
IN USUtelefono VARCHAR(50),
IN USUgenero VARCHAR(50),
IN USUidciudad int
)
begin 
insert into usuario(usuario,correo,usuario.`password`,telefono,genero,ciudad_id_ciudad) values(USUusuario,USUcorreo,USUclave,USUtelefono,USUgenero,USUidciudad);
end */$$
DELIMITER ;
...
/*Table structure for table `verinteresados` */

DROP TABLE IF EXISTS `verinteresados`;

/*!50001 DROP VIEW IF EXISTS `verinteresados` */;
/*!50001 DROP TABLE IF EXISTS `verinteresados` */;

/*!50001 CREATE TABLE  `verinteresados`(
...
/*Table structure for table `verproducto` */

DROP TABLE IF EXISTS `verproducto`;

/*!50001 DROP VIEW IF EXISTS `verproducto` */;
/*!50001 DROP TABLE IF EXISTS `verproducto` */;

/*!50001 CREATE TABLE  `verproducto`(
 `idproducto` int(11) ,
 `nombre` varchar(150) ,
 `cantidad` varchar(50) ,
 `descripcion` varchar(150) ,
...
/*View structure for view verinteresados */

/*!50001 DROP TABLE IF EXISTS `verinteresados` */;
/*!50001 DROP VIEW IF EXISTS `verinteresados` */;

/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `verinteresados` AS (select `interesados`.`idinteresados` AS `idinteresados`,`interesados`.`idproducto_id` AS `idproducto_id`,`interesados`.`idusuario_id` AS `idusuario_id`,`interesados`.`confirmado` AS `confirmado`,`usuario`.`idusuario` AS `idusuario`,`usuario`.`usuario` AS `usuario`,`usuario`.`telefono` AS `telefono`,`usuario`.`correo` AS `correo`,`usuario`.`password` AS `password`,`usuario`.`genero` AS `genero`,`usuario`.`ciudad_id_ciudad` AS `ciudad_id_ciudad`,`ciudad`.`id_ciudad` AS `id_ciudad`,`ciudad`.`ciudad` AS `ciudad`,`ciudad`.`direccion` AS `direccion`,`ciudad`.`departamento_id_departamento` AS `departamento_id_departamento`,`producto`.`idproducto` AS `idproducto`,`producto`.`nombre` AS `nombre`,`producto`.`cantidad` AS `cantidad`,`producto`.`descripcion` AS `descripcion`,`producto`.`precio_estimado` AS `precio_estimado`,`producto`.`marca` AS `marca`,`producto`.`tiempo_uso` AS `tiempo_uso`,`producto`.`usuario_idusuario` AS `usuario_idusuario`,`producto`.`categoria_idcategoria` AS `categoria_idcategoria`,`producto`.`tiempo_publicacion_idtiempo_publicacion` AS `tiempo_publicacion_idtiempo_publicacion`,`producto`.`imagen1` AS `imagen1`,`producto`.`imagen2` AS `imagen2`,`producto`.`imagen3` AS `imagen3`,`producto`.`disponible` AS `disponible`,`producto`.`FechaPublicación` AS `FechaPublicación` from (((`interesados` join `usuario` on(`interesados`.`idusuario_id` = `usuario`.`idusuario`)) join `ciudad` on(`usuario`.`ciudad_id_ciudad` = `ciudad`.`id_ciudad`)) join `producto` on(`interesados`.`idproducto_id` = `producto`.`idproducto`))) */;

/*View structure for view verproducto */

/*!50001 DROP TABLE IF EXISTS `verproducto` */;
/*!50001 DROP VIEW IF EXISTS `verproducto` */;

/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `verproducto` AS (select `producto`.`idproducto` AS `idproducto`,`producto`.`nombre` AS `nombre`,`producto`.`cantidad` AS `cantidad`,`producto`.`descripcion` AS `descripcion`,`producto`.`precio_estimado` AS `precio_estimado`,`producto`.`marca` AS `marca`,`producto`.`tiempo_uso` AS `tiempo_uso`,`producto`.`usuario_idusuario` AS `usuario_idusuario`,`producto`.`categoria_idcategoria` AS `categoria_idcategoria`,`producto`.`tiempo_publicacion_idtiempo_publicacion` AS `tiempo_publicacion_idtiempo_publicacion`,`producto`.`imagen1` AS `imagen1`,`producto`.`imagen2` AS `imagen2`,`producto`.`imagen3` AS `imagen3`,`producto`.`disponible` AS `disponible`,`producto`.`FechaPublicación` AS `FechaPublicación`,`usuario`.`idusuario` AS `idusuario`,`usuario`.`usuario` AS `usuario`,`usuario`.`telefono` AS `telefono`,`usuario`.`correo` AS `correo`,`usuario`.`password` AS `password`,`usuario`.`genero` AS `genero`,`usuario`.`ciudad_id_ciudad` AS `ciudad_id_ciudad`,`categoria`.`idcategoria` AS `idcategoria`,`categoria`.`categoria` AS `categoria`,`ciudad`.`id_ciudad` AS `id_ciudad`,`ciudad`.`ciudad` AS `ciudad`,`ciudad`.`direccion` AS `direccion`,`ciudad`.`departamento_id_departamento` AS `departamento_id_departamento` from (((`producto` join `usuario` on(`producto`.`usuario_idusuario` = `usuario`.`idusuario`)) join `categoria` on(`producto`.`categoria_idcategoria` = `categoria`.`idcategoria`)) join `ciudad` on(`usuario`.`ciudad_id_ciudad` = `ciudad`.`id_ciudad`)) where `producto`.`disponible` = 1 order by `producto`.`idproducto` desc limit 5) */;
